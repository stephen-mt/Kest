...
```text
workloads/cybermarket/
  source/         # schema bootstrap, SQL and transactional event writes
  ingestion/      # generator and upload-before-ack raw CDC writer
  batch/          # Bronze history and finite Silver/Gold publication
  validation/     # smoke tests plus lifecycle and batch verification
  config.py       # environment-backed workload settings
  maintenance.py  # reviewed legacy namespace cleanup
```

Shared S3 and Iceberg primitives live in `src/kest`. Configuration is read
explicitly at startup, so importing a module never requires environment secrets.
...
```sh
make cybermarket-setup              # schema and idempotent dimension seed
make cybermarket-history            # resumable approximately 5 GiB Bronze fixture
make cybermarket-history-reset      # replace only reproducible Bronze objects
make cybermarket-history-deep-check # scan all facts for semantic contradictions
make cybermarket-cdc-test           # emit, land, validate, and stop automatically
make cybermarket-cdc                # foreground CDC; start before generation
make cybermarket-generate           # foreground fixed-rate event generator
make cybermarket-cdc-drain          # land pending changes and exit
make cybermarket-batch              # publish Silver and Gold directly
make cybermarket-batch-check        # validate schemas, counts, and Gold rollups
make cybermarket-airflow            # trigger the same batch through Airflow
make cybermarket-legacy-clean       # list reviewed legacy namespaces
make cybermarket-legacy-purge       # explicitly purge those namespaces
make cybermarket-check              # source schema and operational seed
make cybermarket-history-check      # source plus Bronze manifest
make cybermarket-cdc-check          # source, slot, and raw landing
...
Run `make cybermarket-cdc` and `make cybermarket-generate` in separate terminals for a live demo. Stop
